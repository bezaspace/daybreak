...
- [x] 4.7. **Event schema (MVP):** `task_start`, `agent_start`, `turn_start`, `message_start`, `message_update`, `message_end`, `tool_execution_start`, `tool_execution_end`, `tool_execution_update`, `browser_screenshot`, `auto_retry_start`, `auto_retry_end`, `agent_end`, `bash_execution_update`, `task_complete`, `task_failed`. Additional fields (`approval_request`, `approval_response`, etc.) are added when approval gates are implemented.
