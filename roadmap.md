...
> **Status:** Phase 6 complete. M1-M9 shipped: durable queue, idempotency, retry/dead-letter, tenant quotas, budgets, security hardening, branch/sandbox cleanup, compaction tuning, large-repo resilience, provider fail-fast, and resilience eval fixtures.
...
| [x] 6 | Resilience, Scale & Polish | M1-M9 shipped (queue, idempotency, retry, tenants, budgets, security, cleanup, compaction, provider resilience, evals) |
...
- [x] 4.2. **Agent runner:** Runs inside the E2B sandbox. Wraps Pi SDK, clones the target repo, configures bot committer, iterates (`read`/`bash`/`edit`), then creates a feature branch `daybreak/<task-id>`, commits, and pushes it.
- [x] 4.3. **Sandbox provisioning:** E2B TypeScript SDK `Sandbox.create()` from the `base` template; runtime prep installs git and Node 22. A pre-built `daybreak-browser` E2B template with Node 22, Chromium, and `playwright-core` is also supported for the browser tool.
...
- [x] 4.6. **Persistence:** Supabase Postgres. `sessions`, `tasks`, `events`, `checkpoints` (stub) tables. The control plane persists tasks and events; the UI and REST endpoints read from Supabase.
- [x] 4.7. **Event schema (MVP):** `task_start`, `agent_start`, `turn_start`, `message_start`, `message_update`, `message_end`, `tool_execution_start`, `tool_execution_end`, `tool_execution_update`, `browser_screenshot`, `compaction_start`, `compaction_end`, `auto_retry_start`, `auto_retry_end`, `agent_end`, `bash_execution_update`, `task_complete`, `task_failed`. Additional fields (`approval_request`, `approval_response`, etc.) are added when approval gates are implemented.
- [x] 4.8. **Context compaction:** Integrate Pi's compaction settings and the hard `MAX_TURNS` cap from Phase 0.
...
- [x] 9.1. **Context compaction:** Tune Pi's compaction settings for long tasks and large repositories; per-task overrides via `POST /api/tasks`.
...
- [x] 9.8. **Retry and dead-letter handling:** Failed tasks retry with exponential backoff; permanently failed tasks land in a dead-letter row in Supabase for dead-letter inspection.
...
- [x] 9.12. **Exit criteria:** Attempt to make the agent read `.env` → blocked. Run a task in a loop that never passes tests → hits the 40-turn cap and stops. Simulate a webhook flood → queue absorbs it without duplicate tasks or quota exhaustion.
